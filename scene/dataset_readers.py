...
class CameraInfo(NamedTuple):
    ...
    height: int
    fid: float
    gt_dynamic_mask: Optional[np.array] = None
    depth: Optional[np.array] = None
    rayd: Optional[np.array] = None
...
class SceneInfo(NamedTuple):
    ...
    dynamic_point_cloud: BasicPointCloud
    static_point_cloud: BasicPointCloud
    train_cameras: list
    test_cameras: list
...
def readNerfiesCameras(path):
    ...
    depth_params_path = f'{path}/depth_params.json'
    ...
    if name.startswith('vrig'):
        ...
        if len(val_img) > 0:
            ...
            train_idx = list(range(0, len(dataset_json['ids']), 4))
            val_idx = [idx + 2 for idx in train_idx if idx + 2 < len(dataset_json['ids'])]
            train_img = [dataset_json['ids'][idx] for idx in train_idx]
            val_img = [dataset_json['ids'][idx] for idx in val_idx]
    ...
    def get_time_id(meta_item):
        try:
            return meta_item['time_id']
        except KeyError:
            return meta_item['warp_id']

    all_time = [get_time_id(meta_json[i]) for i in all_img]
    max_time = max(all_time)
    all_time = [get_time_id(meta_json[i]) / max_time for i in all_img]
    ...
    all_mask = all_img
    all_depth = [f'{path}/depth/{int(1/ratio)}x/{i}' for i in all_img]
    all_img = [f'{path}/rgb/{int(1 / ratio)}x/{i}' for i in all_img]
    all_mask = [f'{path}/resized_mask/{int(1 / ratio)}x/{i}.png' for i in all_mask]

    cam_infos = []
    for idx in range(len(all_img)):
        image_path = all_img[idx] + '.png'
        image = np.array(Image.open(image_path))
        image = Image.fromarray((image).astype(np.uint8))
        image_name = Path(image_path).stem
        
        depth_path = all_depth[idx] + '.png'
        depth = np.array(Image.open(depth_path))
        depth = Image.fromarray((depth).astype(np.uint8))

        mask_path = all_mask[idx] + '.png'
        ...
        if depth_params is not None:
            cam_info = CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                                image_path=image_path, image_name=image_name, width=image.size[
                                    0], height=image.size[1],
                                fid=fid, gt_dynamic_mask=mask, pose=1, rayd=1, depth_scale=depth_params[image_name]['scale'], depth_offset = depth_params[image_name]['offset'], depth=depth)
        else:
            cam_info = CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                                image_path=image_path, image_name=image_name, width=image.size[
                                    0], height=image.size[1],
                                fid=fid, gt_dynamic_mask=mask,depth_scale=1.0, depth_offset = 0.0, pose=1, rayd=1)    
        cam_infos.append(cam_info)
    ...
    for i, img_id in enumerate(train_img):
        if i < len(cam_infos):
            img_id_str = str(img_id)
            if img_id_str in img_to_idx:
                train_cameras_infos.append(cam_infos[img_to_idx[img_id_str]])
            else:

                train_cameras_infos.append(cam_infos[i])
    
    for i, img_id in enumerate(val_img):
        if i < len(cam_infos) - len(train_cameras_infos):
            img_id_str = str(img_id)
            if img_id_str in img_to_idx:
                val_cameras_infos.append(cam_infos[img_to_idx[img_id_str]])
            else:
                idx = i + len(train_cameras_infos)
                if idx < len(cam_infos):
                    val_cameras_infos.append(cam_infos[idx])
    
    if len(train_cameras_infos) == 0:
        train_cameras_infos = cam_infos[:train_num]
    if len(val_cameras_infos) == 0 and train_num < len(cam_infos):
        val_cameras_infos = cam_infos[train_num:]
    print(f"Train cameras: {len(train_cameras_infos)}, Val cameras: {len(val_cameras_infos)}")
    sys.stdout.write('\n')
    return cam_infos, train_num, scene_center, coord_scale, train_cameras_infos, val_cameras_infos
    

    # sys.stdout.write('\n')
    # return cam_infos, train_num, scene_center, coord_scale

def readNerfiesCameras2(path):
    with open(f'{path}/scene.json', 'r') as f:
        scene_json = json.load(f)
    with open(f'{path}/metadata.json', 'r') as f:
        meta_json = json.load(f)
    with open(f'{path}/dataset.json', 'r') as f:
        dataset_json = json.load(f)
    # with open(f'{path}/depth_params.json', 'r') as f:
    #     depth_params = json.load(f) 
    ...
    all_mask = all_img
    all_depth = [f'{path}/depth/{int(1/ratio)}x/{i}' for i in all_img]
    # all_depth = [f'{path}/depth/{int(1/ratio)}x/{i}' for i in train_img]
    # all_depth = [f'{path}/depth/{int(1/ratio)}x/{i}' for i in val_img]
    all_img = [f'{path}/rgb/{int(1 / ratio)}x/{i}' for i in all_img]
    all_mask = [f'{path}/resized_mask/{int(1 / ratio)}x/{i}.png' for i in all_mask]
    ...
    for idx in range(len(all_img)):
        ...
        cam_info = CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              image_path=image_path, image_name=image_name, width=image.size[
                                  0], height=image.size[1],
                              fid=fid, gt_dynamic_mask=mask, pose=1, rayd=1, depth=depth)
        cam_infos.append(cam_info)
...
def readNerfiesInfo(path, eval):
    ...
    cam_infos, train_num, scene_center, scene_scale, train_cameras_infos, val_cameras_infos = readNerfiesCameras(path)

    if eval:
        # train_cam_infos = cam_infos[:train_num]
        # test_cam_infos = cam_infos[train_num:]
        train_cam_infos = train_cameras_infos
        test_cam_infos = val_cameras_infos
    else:
        train_cam_infos = cam_infos
        test_cam_infos = []
    ...
    # ply_path = os.path.join(path, "points3d.ply")
    static_ply_path = os.path.join(path, "static_points3d.ply")
    dynamic_ply_path = os.path.join(path, "dynamic_points3d.ply")
    if not os.path.exists(dynamic_ply_path):
        ...
        shs = np.random.random((num_pts, 3)) / 255.0
        dynamic_pcd = BasicPointCloud(points=xyz, colors=SH2RGB(
            shs), normals=np.zeros((num_pts, 3)))

        storePly(dynamic_ply_path, xyz, SH2RGB(shs) * 255)
    try:
        dynamic_pcd = fetchPly(dynamic_ply_path)
    except:
        dynamic_pcd = None
    ...
    if not os.path.exists(static_ply_path):
        ...
        num_pts = xyz.shape[0]
        shs = np.random.random((num_pts, 3)) / 255.0
        static_pcd = BasicPointCloud(points=xyz, colors=SH2RGB(
            shs), normals=np.zeros((num_pts, 3)))
        # num_pts = 20_000
        # print(f"Generating random point cloud ({num_pts})...")
        ...
        storePly(static_ply_path, xyz, SH2RGB(shs) * 255)
    try:
        static_pcd = fetchPly(static_ply_path)
    except:
        static_pcd = None

    scene_info = SceneInfo(dynamic_point_cloud=dynamic_pcd,
                           static_point_cloud=static_pcd, 
                        #    point_cloud=pcd,
                           train_cameras=train_cam_infos,
                           test_cameras=test_cam_infos,
                           nerf_normalization=nerf_normalization,
                        #    ply_path=ply_path,
                           static_path=static_ply_path,
                           dynamic_path=dynamic_ply_path
                           )
